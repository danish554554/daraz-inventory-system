...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      ...
      if (!silent) setState(() => _error = 'Failed to load inventory.');
    } finally {
  ...
  void _optimisticallyUpdateStock({required InventoryItem item, required int delta}) {
    final nextStock = (item.stock + delta).clamp(0, 1 << 31).toInt();
    setState(() {
      _inventory = _inventory.map((row) {
        final sameRow = row.id == item.id || row.inventoryId == item.inventoryId || row.id == item.inventoryId || row.inventoryId == item.id;
        if (!sameRow) return row;
        return row.copyWith(
          stock: nextStock,
          availableStock: (nextStock - row.reservedStock).clamp(0, 1 << 31).toInt(),
  ...
  Future<void> _openRestockSheet({InventoryItem? item}) async {
    ...
    if (delta != null) {
      if (item != null) _optimisticallyUpdateStock(item: item, delta: delta);
      _refreshInventoryQuietly();
      if (mounted) showAppSnackBar(context, 'Stock added successfully. Refreshing quietly in the background.');
    }
  }
  ...
  Future<void> _openQuickAdjustment({InventoryItem? item}) async {
    ...
    if (delta != null) {
      if (item != null) _optimisticallyUpdateStock(item: item, delta: delta);
      _refreshInventoryQuietly();
      if (mounted) showAppSnackBar(context, 'Inventory adjusted successfully. Refreshing quietly in the background.');
    }
  }
  ...
  Future<void> _openBulkRestockSheet() async {
    ...
    if (done == true) {
      _refreshInventoryQuietly();
      if (mounted) showAppSnackBar(context, 'Bulk restock completed. Refreshing quietly.');
    }
  }
          ...
          await showModalBottomSheet<void>(
            ...
            builder: (sheetContext) => InventoryItemActionsSheet(
              ...
              onRestock: () {
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) => _openRestockSheet(item: item));
              },
              onAdjust: () {
                Navigator.pop(sheetContext);
                WidgetsBinding.instance.addPostFrameCallback((_) => _openQuickAdjustment(item: item));
              },
              onRequest: () {
                Navigator.pop(sheetContext);
...
class _QuickAdjustmentSheetState extends State<QuickAdjustmentSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final qty = int.tryParse(_quantity.text.trim()) ?? 1;
      if (mounted) Navigator.pop(context, _type == 'decrease' ? -qty : qty);
    } on ApiException catch (error) {
      if (mounted) showAppSnackBar(context, error.message, error: true);
    } finally {
      if (mounted) setState(() => _saving = false);
